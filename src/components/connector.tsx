import type { ConnectionState, IConnector } from "@rarible/sdk-wallet-connector"
...
type SdkWalletConnectorProps<C> = {
  ...
  children: (conn: C) => ReactElement
}

export function SdkWalletConnector<C>({ connector, children }: SdkWalletConnectorProps<C>) {
        ...
        <div>
          ...
          {options.map(o => (
            <div key={o.option}>
              ...
              {connectionState.status === "connecting" && connectionState.providerId === o.provider.getId()
                ? "Connecting..."
                : null}
            </div>
          ))}
